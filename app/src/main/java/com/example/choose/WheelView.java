...
public class WheelView extends View {
    ...
    private Runnable myRunnable = new MyRunnable();
     ...
     * @param dy
     */
    public void handleMove(float dy) {
        //调整坐标
    ...
    public void showSpecificalItem(int index) {
        /**
         * 调整到指定的item
         */
        ...
        for (int i = 0; i < showCount + 2; i++) {
            int startY = itemHeight * (i - 1);
            int stringIndex = index - showCount / 2 - 1 + i;
            if (stringIndex < 0) {
                stringIndex = lists.size() + stringIndex;
            }
    ...
    public void showSpecificalItemSlide(int index_) {
        /**
         * 调整到指定的item，动态滑动版
         */
        // calculate move steps
        moveSteps = (select - index_ + lists.size()) % lists.size() + lists.size();

        // start dynamic display, slide down
        myRunnable.run();
    ...
    private void SlideDownOneStepAndDisplay() {
        //移除最后一个Item重用
        ...
        for (int i = 0; i < wheelItems.size(); i++) {
            WheelItem item = wheelItems.get(i);
            //设置起点Y坐标
            item.setStartY(item.getStartY() + itemHeight);
        }
        //添加到最开始
